...
local hand_type = function(hand)
    ...
    table.sort(card_counts, function(a, b)
        ...
        return strength[a[1]] > strength[b[1]]
    end)

    local upgrade_J_to = 'J'
    for _, cc in ipairs(card_counts) do
        if (upgrade_J_to == 'J') then
            upgrade_J_to = cc[1]
        end
    end

    for i, c in ipairs(hand) do
        if (c == 'J') then hand[i] = upgrade_J_to end
    end
...
local calulate_winnings = function(input)
    for _, p in ipairs(input) do
        local hand = p[1]
        table.insert(p, hand_type(to_chars(hand)))
        table.insert(p, to_chars(hand))
    end
    table.sort(input, function(a, b)
        if (a[3] ~= b[3]) then return (a[3] > b[3]) end
        return compare_cardwise(a[4], b[4])
    end)
    ...
    for i = 1, #input do
        ...
        print('Hand ' .. p[1] .. ', rank ' .. rank .. ', type(' .. p[3] .. ')')
